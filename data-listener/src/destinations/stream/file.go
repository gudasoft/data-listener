...
func (cfg FileStreamConfig) String() string {
	filePath, fileName := cfg.generateFilePathAndName()
	return path.Join(filePath, fmt.Sprintf("%s.%s, uniqueFilePerRequest: %t, maxFileSize: %dkB", fileName, cfg.FileExtension, cfg.UniqueFilePerRequest, cfg.MaxFileSize/1024/1024))
}
